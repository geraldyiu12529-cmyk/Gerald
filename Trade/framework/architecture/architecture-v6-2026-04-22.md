# T.system Architecture — v6
**Version:** 6  
**Dated:** 2026-04-22  
**Prior version:** `architecture-v5-2026-04-22.md`  
**Supersedes:** v5 (for consumer reads)

**Changes from v5:**
- **§2 Key Documents / §4 Skills / §5 architecture-health / §6 Mermaid / governance:** skill renamed `methodology-sync` → `architecture-health` throughout — Maintained by line, §2 Methodology Prompt Loaded-by column, §4 Skills table row, §5 section heading and all wiring rows (reads, writes, patches), §6 Mermaid MS node label and skill path.
- **§9 Changelog:** rename row prepended.
...
**Maintained by:** `architecture-health` skill — cuts a new version when skill wiring, file paths, or task scheduling changes.  
...
|---|---|---|---|
| Methodology Prompt | `framework/Methodology Prompt.md` | 8-step framework, Top-33 variables (A/B/C grades), S/T/C/R scoring rules, Overlay Gate | market-brief, daily-trade-rec, signal-review, quarterly-review, architecture-health |
...
| Master Data Log | `Trade/master-data-log.xlsx` | 11 sheets: SignalLedger, PerformanceStats, RegimeHistory, DailyVariables, AuditAdditionLog, DataQuality, VariableRegistry, MethodologyNotes, CatalystLog, HypoLedger, README | 7+ skills read or write |
...
| `system-review-{date}.md` + `.html` | system-review | architecture-health | Semi-annual, permanent |
...
All under `~/.claude/skills/{skill-name}/SKILL.md`.
...
| `architecture-health` | Sonnet | On-demand (after Methodology Prompt changes) |
| `pipeline-smoketest` | — | On-demand diagnostic (read-only) |
| `asset-universe-update` | — | On-demand (universe changes only) |
| `skill-manager` | Haiku | On-demand (skill lifecycle management) |
...
| ← reads | HypoLedger (dedup) | `Trade/master-data-log.xlsx` |
...
| → appends | HypoLedger (Step 8.5 — promoted signals at rec price) | `Trade/master-data-log.xlsx` |
...
| ← reads | HypoLedger (all rows) | `Trade/master-data-log.xlsx` |
| ← reads | Memory | `Trade/framework/Memory.md` §2, §5 |
...
| → writes | Signal review (incl. §H HypoLedger Efficacy Report) | `Trade/{YYYY-MM-DD}/signal-review-{YYYY-MM-DD}.md` |
| → updates | HypoLedger (mark-to-market + dim-14 efficacy) | `Trade/master-data-log.xlsx` |
| → updates | PerformanceStats (14 dimensions), VariableRegistry | `Trade/master-data-log.xlsx` |
| → appends | memory-lessons | `Trade/framework/memory-lessons.md` |
...
| Direction | File | Path |
|---|---|---|
| ← reads | All SKILL.md files | `~/.claude/skills/*/SKILL.md` |
...
| Direction | File | Path |
|---|---|---|
| ← reads | Global CLAUDE.md (current universe) | `~/.claude/CLAUDE.md` |
| ← reads | All SKILL.md files | `~/.claude/skills/*/SKILL.md` |
...
| → writes | Sync report | `Trade/{YYYY-MM-DD}/architecture-health-{YYYY-MM-DD}.md` |
| → patches | Stale SKILL.md files (CRITICAL urgency, with Gerald sign-off) | `~/.claude/skills/*/SKILL.md` |
...
### Workspace canonicalization (effective 2026-04-22)
- **Canonical workspace:** `C:\Users\Lokis\OneDrive\Desktop\T.system\Trade\`
- **Skills location:** `~/.claude/skills/` (14 skills; moved from `Trade/.claude/skills/` 2026-04-22)
- **`.claude` directories:** only 1 remains (`~/.claude/` global); `Trade/.claude/` deleted 2026-04-22 (rules, settings, and CLAUDE.md consolidated into global `~/.claude/`)
- **`cowork/` directory:** deleted 2026-04-22 (cowork platform workspace removed)
