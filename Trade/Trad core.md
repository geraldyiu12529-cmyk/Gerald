...
## Addendum — 2026-04-18 meta-analysis integration additions

Five new variables entered the framework after the 2026-04-18 meta-analysis synthesis (BNMA × 4 + PL-NMA of 54 variables × 12 papers + systematic reviews × 10). See `meta-analysis-integration-plan-2026-04-18.md`. Four enter at Grade A; one (V030 DealerGamma) enters at Grade B pending second independent replication. All five share 2026-10-14 six-month review date with the 2026-04-14 audit cohort. Each carries McLean-Pontiff post-publication decay risk; expect 30–50% Sharpe haircut vs. published.

### 29. V029 BAB — Betting-Against-Beta (single-stock + ETF sleeve) — Grade A

**Concept.** Long low-β / short high-β zero-cost portfolio, leveraged to β=1. Exploits the CAPM-violation premium: high-β stocks are systematically overpriced, low-β systematically underpriced, because leverage-constrained investors bid up high-β instead of levering low-β exposure.

**Primary citation.** Frazzini, A., & Pedersen, L. H. (2014). *Betting Against Beta.* Journal of Financial Economics 111(1), 1–25.

**Replication.** Buchner, A., & Wagner, N. (2017). *The Betting Against Beta Anomaly: Fact or Fiction?* Finance Research Letters 20, 1–6 (confirmed with caveats on leverage-cost assumptions). Asness, Frazzini, Pedersen (2020) robustness checks across regions and time periods.

**Mechanism.** Leverage aversion binds. Mutual funds, retail accounts, and most institutional mandates cannot freely lever cash positions to reach a desired exposure level, so they over-concentrate in high-β names to reach target returns. This persistent demand elevates high-β prices relative to CAPM and suppresses low-β prices. The premium is structural, not behavioral, and therefore unlikely to fully arbitrage away.

**What it adds.** Independent of V009 TSMOM (momentum is about *trend*; BAB is about *beta level*). Independent of V026 Residual momentum (which strips β; BAB trades β). Orthogonal to value/quality factors. Meta 2026-04-18 PL-NMA rank 6/54, θ=+1.60; 4 papers, single-group "Other" with no peer shrink in BNMA.

**Scope.** Single-stock equities (all 12 named tickers) and ETF proxies (USMV/SPLV spread for tactical reads). Runs as an **independent factor sleeve** capped at 1/3 of V009's risk budget. Do NOT aggregate into the V009/V027/V028 spine sizing. See `Risk Rules.md §8`.

**Caveats.** (a) ETF-proxy implementation captures only a fraction of canonical BAB's leveraged long/short β-decile structure — adequate for tactical scoring but not for 2026-10-14 review grading (canonical AQR data required); (b) post-2018 underperformance flagged in GWZ (2024); still Grade A on long-sample evidence; (c) regime-sensitive to funding-rate shocks; (d) post-decay Sharpe projected 0.4–0.8.

### 30. V030 DealerGamma — Options dealer gamma positioning (Risk overlay) — Grade B
...
**Mechanism.** Market-makers writing options must delta-hedge continuously. When dealer inventory is net-short gamma (dealers wrote more options than they bought), small index moves force them to buy at the top and sell at the bottom — a pro-cyclical rehedging flow that amplifies realized volatility. Net-long gamma is the mirror image: dealers sell into rallies and buy into dips, damping moves. Empirically, the sign of aggregate dealer gamma predicts next-day realized/implied vol ratio and S&P intraday volatility.

**What it adds.** Cleanly decoupled from V009 TSMOM in meta-analysis BNMA runs; PL-NMA rank 5/54 primary and rank 5 S2; θ=+1.68. Complementary to V001 VIX and V027 Intermediary Capital — V001 is an implied-vol level, V027 is a capacity constraint, DealerGamma is a *flow-structure* modifier that changes how the overlay translates to stop widths.

**Scope.** Risk overlay modifier. Short-gamma regime → widen R stop by one notch; long-gamma → tighten R stop by one notch. Also participates in the R-group scale-consistency rule (`Risk Rules.md §5.A`): when co-firing with V001 VIX, take the more-negative.

**Caveats.** (a) Single-paper status — retain Grade B until second replication, do not promote to A before 2026-10-14 review; (b) SqueezeMetrics / SpotGamma data is paid, subscription-dependent — the pull may be permanently MISSING if subscription not active (see `Data Sources.md` V030 row); (c) dealer positioning inference methodology differs between providers — document which provider is used; (d) mechanism may change if options-market structure changes (e.g., 0DTE share of volume continues to grow); (e) post-decay Sharpe projected 0.4–0.7.

### 31. V031 GP/A — Gross Profitability / Assets (single-stock) — Grade A

**Concept.** Ratio of gross profits (revenue minus COGS) to total assets, re-balanced quarterly and used to rank single stocks cross-sectionally. High-GP/A stocks earn a persistent premium over low-GP/A stocks, even controlling for value, size, and market beta.

**Primary citation.** Novy-Marx, R. (2013). *The Other Side of Value: The Gross Profitability Premium.* Journal of Financial Economics 108(1), 1–28.

**Replication.** Integrated into the Fama-French 5-factor model (Fama & French 2015, *JFE* 116(1), 1–22) as RMW (Robust-Minus-Weak profitability), which uses operating profitability (nearly collinear with GP/A at horizons of months+). Ball et al. (2015) confirm gross profitability is the highest-signal-to-noise profitability measure.

**Mechanism.** Gross profitability is the "other side of value" — value sorts on *price*, profitability sorts on *quality*. The two are nearly orthogonal and both earn premia. Mechanism is a compensation for sustained earnings power not impounded by short-horizon investor attention. Accounting-level transformations (e.g., depreciation choices) do not affect gross profit, making GP/A the cleanest profitability variant for cross-sectional ranking.

**What it adds.** Canonical Fama-French 5F component, already orthogonalized against V009 TSMOM, V026 Residual momentum, V029 BAB, and valuation proxies. Meta 2026-04-18 PL-NMA rank 15/54, θ=+0.63; multiple systematic reviews confirm Grade A.

**Scope.** Single-stock Structural (Step 2) input on all 12 named tickers. Ranked monthly; rebalance on monthly basis (not daily).

**Caveats.** (a) Quarterly financial-statement data has ~30–45 day reporting lag — use the *previously reported* GP/A, not current-quarter estimates; (b) sector-sensitive — compare tech GP/A to tech peers, not to staples; consider within-sector decile ranks for single-stock use; (c) post-decay Sharpe projected 0.3–0.5 (modest but robust and cheap to implement); (d) JKP-tangency implementation with HMLDevil and QMJ planned when those variables are promoted at 2026-07-01 or later.

### 32. V032 CEI — Composite Equity Issuance (single-stock) — Grade A

**Concept.** Net share issuance over a lookback window, normalized by market cap. A firm that issues a lot of equity (secondary offerings, acquisitions-for-stock, employee compensation expense) is effectively disinvesting from the business — the net-issuance rank predicts cross-sectional underperformance on a negative-sign basis.

**Primary citation.** Daniel, K., & Titman, S. (2006). *Market Reactions to Tangible and Intangible Information.* Journal of Finance 61(4), 1605–1643.

**Replication.** Pontiff & Woodgate (2008) *JF* 63(2), 921–945 confirm the signal on a long post-publication out-of-sample window. Fama-French (2008) *JF* 63(4), 1653–1678 include net-issuance as a robust cross-sectional return predictor.

**Mechanism.** Management issues equity when it believes shares are overvalued (or at least fairly valued, given the dilution cost) and uses equity for acquisitions or compensation. The signal is a composite of (a) manager information advantage, (b) real-investment disinvestment (issuance proceeds often fund lower-ROIC projects), and (c) overvaluation reversals. Negative sign: high issuance = structural headwind; low or net-buyback = structural tailwind.

**What it adds.** Independent of V024 Buyback/net-payout signal (already in Top-25 as Grade B) — CEI is the *comprehensive* net-issuance measure including dilution, not just buyback activity. Independent of V031 GP/A (issuance is about corporate actions; GP/A is about operational efficiency). Meta 2026-04-18 PL-NMA rank 7/54, θ=+1.33.

**Scope.** Single-stock Structural (Step 2) input on all 12 named tickers. Self-computed from CRSP share-count and Compustat data; quarterly rebalance.

**Caveats.** (a) Self-compute only — no pre-packaged free source; `scripts/compute_cei.py` to be written in Phase 2; (b) reporting lag on both CRSP and Compustat — use trailing-4Q window; (c) can fire spuriously during tech IPO-heavy years when issuance is structural, not valuation-signal; (d) post-decay Sharpe projected 0.4–0.6; (e) interpret negative-sign carefully — a stock with high CEI is a *short-bias* signal in a long-only framework, so it down-weights the single-stock S-input.

### 33. C009 Faber TAA — 10-month SMA sleeve gate (V033 SPY / V034 GSCI / V035 BTC) — Grade A

**Concept.** Rule: if the sleeve price is above its 10-month simple moving average at the previous month-end close, the sleeve is "on" for the following month; if below, the sleeve is gated OFF. A drawdown circuit-breaker, not an alpha signal.

**Primary citation.** Faber, M. (2007). *A Quantitative Approach to Tactical Asset Allocation.* Journal of Wealth Management 9(4), 69–79.

**Replication.** Asness, Frazzini, Pedersen (2013) sections on simple trend-following variants. Hurst, Ooi, Pedersen (2017) *JPM* systematic trend-following replications. ReSolve Asset Management and MebFaber (author) continuously updated OOS tests across the post-2007 window.

**Mechanism.** Not an alpha variable in the traditional sense. Faber TAA is an empirical observation that monthly 10-SMA crossovers catch the start of large drawdowns early enough to avoid the deep tail (−40%+ declines), at the cost of whipsaw losses during sideways markets. The net effect is Sharpe-neutral to slightly-positive but *drawdown-negative by design*: maximum drawdowns are 1/2 to 1/3 of buy-and-hold. Mechanism is institutional: slow-moving money (401(k), endowments) unwinds in months-long arcs, creating persistent trends at monthly resolution.

**What it adds.** Fills the 8-step's previously absent **Overlay Regime Gate** layer — a dedicated scope for non-additive sleeve-level risk management. Non-additive to Sum (`Methodology Prompt.md §Step 1.5`). Meta 2026-04-18 PL-NMA rank 2/54, θ=+2.26; multi-review DEPLOY.

**Scope.** Three asset instantiations — V033 SPY (equity-sleeve gate for QQQ/SPY and by extension the 12 single-stock longs), V034 GSCI (commodity-sleeve gate), V035 BTC (crypto-sleeve gate). Optional fourth: EFA for international-equity sleeve (EWJ/EWY).

**Caveats.** (a) Sleeve flips happen at end-of-month only — gate reads previous month-end close against its 10m-SMA for the following month; do not recompute intraday; (b) whipsaw risk at monthly boundaries — strict adherence to the monthly cadence is mandatory; (c) as a gate, it can only reduce risk, never increase it — it does not generate buy signals, only blocks existing long-Sum signals in their sleeves; (d) gate state interacts with the −15% / −20% portfolio-level circuit breakers in `Risk Rules.md §4` — both apply, take the more restrictive.
