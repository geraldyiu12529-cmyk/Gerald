...
```json
{
  ...
  "market-brief": {
    ...
    "missing_count": 1,
    "file": "market-brief-2026-04-16.md"
  },
  "news-events": {
    "date": "2026-04-16",
    "status": "OK",
    ...
    "file": "news-events/news-2026-04-16.md"
  },
  "trade-rec": {
    "date": "2026-04-16",
    "status": "OK",
...
3. After completing Phase 1 and Phase 2, before exiting:
   ...
   b. Verify `audit-data-staging-{today}.md` was written and is non-empty
   c. Write preflight status to `.pipeline-status.json`:
      ...
      staging_ok = Path(f'/mnt/Trade/audit-data-staging-{today}.md').exists()
...
status_path = Path('/mnt/Trade/.pipeline-status.json')
status = json.loads(status_path.read_text()) if status_path.exists() else {}
brief_path = Path(f'/mnt/Trade/market-brief-{today}.md')
...
status_path = Path('/mnt/Trade/.pipeline-status.json')
status = json.loads(status_path.read_text()) if status_path.exists() else {}
news_path = Path(f'/mnt/Trade/news-events/news-{today}.md')
...
status['news-events'] = {
    'date': today, 'status': 'OK' if ok else 'FAIL',
    'consecutive_failures': consec, 'timestamp': now_iso,
    'file': f'news-events/news-{today}.md'
}
status_path.write_text(json.dumps(status, indent=2))
```
```
...
Check `/mnt/Trade/` for `market-brief-{date}.md` files covering those 5 days.
...
2. For each task, check today's status:
   ...
   → Recovery: attempt to produce a minimal "cache-brief" from cached data:
     ...
     c. If ≥20 variables available from cache: write a market-brief-{today}-recovery.md with STALE tags on every reading
   ...
   → News is supplementary. Write a skeleton news-{today}-recovery.md with "NEWS CAPTURE FAILED — no category searches completed. Trade rec should proceed with C-leg gaps."
