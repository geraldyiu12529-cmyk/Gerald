...
**On-demand phrases:** "system review", "strategic review", "review the system", "audit my skills", "audit my architecture", "optimize the system", "reduce token burn", "is the architecture working", "review my workspace strategically", "meta review".

---

## Why this skill exists

No existing skill evaluates whether the trading scaffold itself is working. `quarterly-methodology-review` evaluates methodology content. `signal-review` evaluates signal quality. `workspace-tidy` does tactical hygiene. `skill-creator` executes on-demand edits. None of them ask: given the earn-money aim, is the *architecture* — the set of skills, tasks, and supporting files — correctly shaped AND efficiently run?
