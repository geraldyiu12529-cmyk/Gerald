...
Produce a **compact** HTML report. Required sections only:
...
Omit: all Chart.js charts, score stacking charts, time-series sparklines, gap-closure doughnut, BTC address chart, yield curve chart. On a flat day the HTML is an audit record, not a decision aid.

### If `signal_count` ≥ 1 (active signal day):

Produce the **full** HTML report as specified below.
