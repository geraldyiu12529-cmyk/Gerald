...
description: Cloud US pre-open market brief — 6pm UTC+8, full methodology, uploads MD to Google Drive + Slack notify
---
...
Always pull:
- VIX (Whaley 2009 — B; downgraded 2026-04-18 BNMA: regime-sensitive, use as context not primary trigger)
- MOVE index (Choi-Mueller-Vedolin 2017 — A)
- DXY, US 2Y yield, US 10Y yield, 10Y breakeven inflation (A)
- 10Y TIPS real yield (V007 — B; downgraded 2026-04-18: cross-asset correlation with equity E/P varies by inflation regime)
- CDX HY spread in bp (Gilchrist-Zakrajšek 2012 — B; downgraded 2026-04-18: correlates 0.65–0.75 with V027 post-2008, confirmatory not independent signal)
- NFCI (A)
- SPY, QQQ, EWJ, EWY prices
- All equity universe: INTC TSM NVDA TSLA AAPL GOOGL AMZN META PYPL PLTR MU WDC AVGO BABA MSFT
- Commodities: Brent, WTI, Gold, Silver, Copper, Palladium, Platinum
...
- FX carry direction (V015, Grade A — context only, no scorecard rows): G10 rate differential direction. Widening = carry-supportive; narrowing/reversing = carry-unwind = cross-asset risk-off. Note EM carry stress if present.
- FX real valuation (V016, Grade B/A — context only): DXY vs PPP fair value; note if extreme dollar overvaluation is building commodity/equity headwind.
...
4. Financial conditions: NFCI, CDX HY, DXY, FX carry direction (V015) → Tight / Neutral / Loose. Flag carry-unwind if G10 rate differentials narrowing.
...
T — Tactical:
- Indices/ETFs/commodities/crypto: raw 12-1m time-series momentum (A)
...
- Intermediary capital ratio (NY Fed PD z-score — A, Adrian-Etula-Muir 2014 JF 69(6) [primary]; He-Kelly-Manela 2017 JFE [secondary]): z < −1σ → downgrade R one notch cross-asset. DOUBLE-COUNT GATE: if CDX HY also flagging stress simultaneously, count once — take the more negative. If V027 turns negative BEFORE CDX HY widens, apply the notch on V027 alone.
- V030 DealerGamma modifier (B — Barbon-Buraschi 2021; single-paper, second replication pending): short-gamma → widen R stop by one notch; long-gamma → tighten. DOUBLE-COUNT GATE: do not double-count with VIX when both flag stress. Source: SqueezeMetrics GEX → SpotGamma → MISSING. Flag single-paper status whenever cited.
...
Compose the brief with these sections:
  ...
  # Market Brief — YYYY-MM-DD (18:00 UTC+8) [Cloud-6pm]
...
- title: `market-brief-{YYYY-MM-DD}.md` (today's date in UTC+8)
- mimeType: `text/plain`
...
After upload, send ONE message to #trading-scheduled-updates (Channel ID: C0AUCTQSC65, private).
If the ID fails, re-resolve via slack_search_channels(query="trading-scheduled-updates", channel_types="public_channel,private_channel").

  [CLOUD-BRIEF] YYYY-MM-DD 18:00 UTC+8
  Regime: {composite one-liner}
  VIX {x} (B) | MOVE {x} (A) | DXY {x} (A) | US10Y {x}% (A) | CDX HY {x}bp (B)
  Sleeves: equity {ON/OFF} | commodity {ON/OFF} | crypto {ON/OFF} | intl {ON/OFF}
...
- B = regime-dependent or regime-sensitive (VIX, CDX HY/real yields, news sentiment, funding, MVRV, DealerGamma). Filter/context only, never primary trigger. Downgraded 2026-04-18: V001 VIX, V004 HY OAS, V006 2s10s, V007 real yields, V008 ACM term premium.
