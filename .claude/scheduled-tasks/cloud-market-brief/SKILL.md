---
name: cloud-market-brief
description: Cloud pre-open market brief — weekdays 18:00 UTC+8, Grade A variables, 8-step methodology, uploads to Google Drive + Slack.
---

**Workspace anchor (run FIRST — context verification only; this task writes to Google Drive, not local disk):**
...
Canonical workspace: `T.system/Trade/`. This task is cloud-only — outputs go to Google Drive (not the local date folder). Do NOT write to `cowork/Gerald/cloud-sync/`.
...
- **FX carry direction (V015, Grade A — context only, no scorecard rows):** G10 rate differential direction (e.g., USD/JPY carry, AUD/JPY carry spread). Widening = carry-supportive; narrowing/reversing = carry-unwind = cross-asset risk-off. Note EM carry stress if present.
...
- Intermediary capital ratio (NY Fed PD z-score — A, Adrian-Etula-Muir 2014 JF 69(6) [primary anchor]; He-Kelly-Manela 2017 JFE [secondary]): z < −1σ → downgrade R one notch cross-asset (equities and commodities). DOUBLE-COUNT GATE: if CDX HY also flagging stress simultaneously, count once — take the more negative of the two, not their sum. If V027 turns negative BEFORE CDX HY widens, apply the notch on V027 alone.
